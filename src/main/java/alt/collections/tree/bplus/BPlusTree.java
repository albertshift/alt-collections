...
public final class BPlusTree<K, V> implements Tree<K, V> {
	...
	public BPlusTree(Configuration<K> conf) {
		this.conf = conf;
		this.root = new LeafNode<K, V>(conf);
	}
	
	public static <K, V> BPlusTree<K, V> newInstance(int branchingFactor, final Comparator<? super K> keyComparator) {
		return new BPlusTree<K, V>(Configuration.newInstance(branchingFactor, keyComparator));
	}
